...
describe('Construct Release API', () => {
  it('should return 400 for invalid date format', async () => {
    const response = await fetch('http://localhost:5173/api/constructRelease?date=invalid-date&releaseDefinitionId=123');
...
describe('Construct Release API', () => {
  it('should return 400 for missing releaseDefinitionId', async () => {
    const response = await fetch('http://localhost:5173/api/constructRelease?date=2023-01-01');
    expect(response.status).toBe(400);
    const data = await response.json();
...
describe('Construct Release API', () => {
  it('should not return 500 due to missing env vars', async () => {
    const response = await fetch('http://localhost:5173/api/constructRelease?date=2023-01-01&releaseDefinitionId=123');
