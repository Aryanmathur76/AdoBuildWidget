...
export async function GET({ url }: { url: URL }) {
    ...
    if (!releaseId) {
        return json({ error: 'Missing releaseId parameter' }, { status: 400 });
    }
    ...
    const runsUrl = `https://dev.azure.com/${AZURE_DEVOPS_ORGANIZATION}/${AZURE_DEVOPS_PROJECT}/_apis/test/runs?releaseIds=${releaseId}&minLastUpdatedDate=${encodeURIComponent(minLastUpdatedDate)}&maxLastUpdatedDate=${encodeURIComponent(maxLastUpdatedDate)}&api-version=7.1`;
